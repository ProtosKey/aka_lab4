...
**Студент:** `Газизуллин Ринат Ришатович | P3216 | 465487`

**Вариант:** `lisp | risc | harv | mc | tick | binary | stream | port | cstr | prob2 | superscalar`

> Усложнение `superscalar` не реализовано.
...
**Вычисление:** аппликативный порядок, слева направо.

**Область видимости:** два уровня (глобальный и локальный функции).

**Типы:** знаковое целое (4 байта), строки (адреса), булевы (ноль или любое), сравнения (возвращают 0 или 1).
...
| `load` | 1 | слово из `data memory` |
| `store` | 2 | сохранение в `data memory` |
| `load-byte` | 1 | расширенный байт из `data memory` |
| `store-byte` | 2 | сохранения байта в `data memory` |
| `halt` | 0 | окончание симуляции |
...
x5–x7      — временные, caller-saved
x8  (fp)   — указатель фрейма, callee-saved
x10–x17    — аргументы или возвращаемое значение (a0–a7)
...
```
0x0000
1) boot stub (5 инструкцций) - LUI/ADDI gp, LUI/ADDI sp, JAL main
0x0014
2) тела функций (в порядке defun)
3) main - тело программы, оканчивается HALT
```

Загружается из `.bin` перед запуском в формате `little-endian`.

### 2.3 Память данных (data memory, 64 KiB)

```
0x0000
строковые литералы - NUL-terminated, байт на символ
GP_BASE
глобальные переменные - 4 байта на переменную в порядке первого появления
Свободная память
0xFFFC
...
Загружается из `.data.bin` перед запуском в формате `little-endian`.
...
RISC-архитектура в стиле RV32I. Все инструкции 32 битные, фиксированная ширина.
...
`imm` знакорасширяется до 32 бит.

### 3.2 Набор инструкций

| Мнемоника | Формат | Количество тиков | Операция |
|---|:---:|:---:|---|
...
| `ADDI/ANDI/ORI/SLLI/SRLI` | I | 2 | то же с `imm` |
...
```
prog.bin       — сырой бинарник, 4 байта на инструкцию, `little-endian`
prog.data.bin  — начальное содержимое data memory
prog.lst       — дизассемблер (формат: <ADDRESS> - <HEXCODE> - <mnemonic>)
```
