...
def _pack_i(opcode, rd, f3, rs1, imm):
    return (
        (opcode & 0x7F)
        | ((rd  & 0x1F) << 7)
        | ((f3  & 0x7)  << 12)
...
def _pack_r(opcode, rd, f3, rs1, rs2, f7):
    return (
        (opcode & 0x7F)
        | ((rd  & 0x1F) << 7)
        | ((f3  & 0x7)  << 12)
...
def _pack_s(opcode, f3, rs1, rs2, imm):
    return (
        (opcode & 0x7F)
        | ((imm & 0x1F) << 7)
        | ((f3  & 0x7)  << 12)
...
def _pack_b(opcode, f3, rs1, rs2, imm):
    b = imm
    return (
        (opcode & 0x7F)
        | (((b >> 11) & 0x1) << 7)
...
def _pack_u(opcode, rd, imm20):
    return (opcode & 0x7F) | ((rd & 0x1F) << 7) | ((imm20 & 0xFFFFF) << 12)

def _pack_j(opcode, rd, imm):
    b = imm
    return (
        (opcode & 0x7F)
        | ((rd & 0x1F) << 7)
...
class Assembler:
    ...
    def addi(self, rd, rs1, imm):
        self._raw(_pack_i(_OP_IMM, rd, 0, rs1, imm),
                  f"addi {_rn(rd)}, {_rn(rs1)}, {imm}")

    def add(self, rd, rs1, rs2):
        self._raw(_pack_r(_OP, rd, 0b000, rs1, rs2, 0b0000000),
                  f"add {_rn(rd)}, {_rn(rs1)}, {_rn(rs2)}")

    def sub(self, rd, rs1, rs2):
        self._raw(_pack_r(_OP, rd, 0b000, rs1, rs2, 0b0100000),
                  f"sub {_rn(rd)}, {_rn(rs1)}, {_rn(rs2)}")

    def mul(self, rd, rs1, rs2):
        self._raw(_pack_r(_OP, rd, 0b000, rs1, rs2, 0b0000001),
                  f"mul {_rn(rd)}, {_rn(rs1)}, {_rn(rs2)}")

    def lui(self, rd, imm20):
        self._raw(_pack_u(_LUI_OP, rd, imm20),
                  f"lui {_rn(rd)}, 0x{imm20:05X}")

    def lw(self, rd, rs1, imm):
        self._raw(_pack_i(_LOAD, rd, 0b010, rs1, imm),
                  f"lw {_rn(rd)}, {imm}({_rn(rs1)})")

    def lb(self, rd, rs1, imm):
        self._raw(_pack_i(_LOAD, rd, 0b000, rs1, imm),
                  f"lb {_rn(rd)}, {imm}({_rn(rs1)})")

    def sw(self, rs1, rs2, imm):
        self._raw(_pack_s(_STORE, 0b010, rs1, rs2, imm),
                  f"sw {_rn(rs2)}, {imm}({_rn(rs1)})")

    def sb(self, rs1, rs2, imm):
        self._raw(_pack_s(_STORE, 0b000, rs1, rs2, imm),
                  f"sb {_rn(rs2)}, {imm}({_rn(rs1)})")
    ...
    def jal(self, rd, label: str):
        base = _pack_j(_JAL_OP, rd, 0)
        self._fixup_raw(base, label, f"jal {_rn(rd)}, {label}", "J")

    def jalr(self, rd, rs1, imm=0):
        self._raw(_pack_i(_JALR_OP, rd, 0, rs1, imm),
                  f"jalr {_rn(rd)}, {_rn(rs1)}, {imm}")

    def in_port(self, rd, port):
        self._raw(_pack_i(_IO_IN, rd, 0, X0, port),
                  f"in {_rn(rd)}, {port}")

    def out_port(self, rs1, port):
        self._raw(_pack_s(_IO_OUT, 0, rs1, X0, port),
                  f"out {_rn(rs1)}, {port}")
    ...
    def listing(self) -> list[ListingEntry]:
        words = self.resolve()
        return [
            ListingEntry(i * 4, words[i], self._mnems[i])
